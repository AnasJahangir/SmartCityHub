...
const Sidebar = () => {
  ...
  return (
    ...
    <div className="d-flex  justify-between lg:block xs:hidden sm:hidden items-center text-black">
      ...
      {sidebarOpen ?  <AiOutlineClose size={30} /> : <AiOutlineBars size={30}  onClick={toggleSidebar}/>}
      </div>
    </div>

    {/* Sidebar */}
    <div className={`flex-col text-black h-full fixed top-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} 
    md:translate-x-0 md:relative md:flex md:w-64 transition-transform duration-300 ease-in-out`}>
      ...
      <div className="lg:hidden block text-lg font-bold mt-6 p-0 ml-6">
        ...
        <div className="text-lg font-bold mt-6 p-0 ml-6">
          Control Panel
        </div>
    ...
    {/* Main Content */}
    <div className={`flex-1 ${sidebarOpen ? 'ml-64' : ''} md:ml-64 p-4 transition-all duration-300`}>
      <Outlet />
    </div>
  </div>
  <House/>
